...
class Bro {
  ...
  ///opponentName : name of the bro this peer is currently playing with, empty if free ("idle")
  String opponentName;
...
//
  factory Bro.fromJson(Map<String, dynamic> json) {
    ...
    //globalData.logger.i('lAppLifState $lAppLifState - ${json['lifecycle_state'] ?? 'null'}\n');
    return Bro(
  ...
  ///isBusy : true if this bro is currently occupied with a game (request/build/play), not free for a new one
  bool isBusy() {
    return opponentName.isNotEmpty;
  }
...
String getStatusAsString() {
  ...
  if (myStatus == Status.onHold) {
    return 'onHold';
  }
  if (myStatus == Status.blocked) {
    return 'blocked';
  }
  return 'unknown';
}
  ...
  ///setOnHold : bro missed a heartbeat, give it one more chance via a direct probe
  setOnHold() {
    myStatus = Status.onHold;
  }
  ...
  Bro? getBroByKey(String iKey) {
    for (var i = 0; i < bros.length; i++) {
      if (bros[i].publicKey == iKey) {
        //globalData.logger.i('getBroByKey bro found index $i\n');
        return bros[i];
      }
    }
    //globalData.logger.i('getBroByKey not bro found key: $iKey\n');
    return null;
  }
